...
def train(dmodel, doptimizer, emodel, eoptimizer, train_loader, num_steps=100):
    ...
    for epoch in range(1, num_steps + 1):
        ...
        for x0, _ in train_loader:
            ...
            slogp = dmodel(xt, t_time, mu) 
            score = torch.mean((slogp - logp) ** 2)
            l1 = norml1(mu)
            loss = lmda(t_val, type="VP") * score + gamma * l1
            loss.backward()
            doptimizer.step()
            eoptimizer.step()
...
torch.save(dmodel.state_dict(), PATH_DIR / "denoiserDRLbeta.pt")
torch.save(emodel.state_dict(), PATH_DIR / "encoderDRLbeta.pt")
